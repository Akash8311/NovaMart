...
const DETAILS = [
  ...
  { label: "Manufacturer",          value: "NovaMART Brand — Symbol" },
  { label: "ASIN",                  value: "B09PRKQYCX" },
  { label: "Item Model Number",     value: "AW17MFS229" },
  { label: "Country of Origin",     value: "India" },
  { label: "Department",            value: "Women" },
  { label: "Item Weight",           value: "350 g" },
  { label: "Generic Name",          value: "Dress" },
  { label: "Net Quantity",          value: "1 unit" },
  { label: "Packer",                value: "CLOUDTAIL INDIA PVT. LTD., H-9 Block B-1, Mohan Cooperative Industrial Area, Mathura Road, New Delhi – 110044" },
];

const FEATURES = [
  { icon: "🌿", title: "Eco Fabric",    desc: "Made from sustainably sourced materials with low environmental impact." },
  { icon: "✂️", title: "Tailored Fit",  desc: "Precision cut for a flattering silhouette across all size ranges." },
  { icon: "🧼", title: "Easy Care",     desc: "Machine washable. Retains colour and shape after multiple washes." },
  { icon: "🚚", title: "Fast Delivery", desc: "Ships within 24 hours. Free delivery on orders above ₹499." },
];

function DescriptionTab() {
  const [expandPacker, setExpandPacker] = useState(false);
  return (
    <div style={{ padding:"32px 36px", animation:"fadeUp .4s ease both" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:0 }}>
        {DETAILS.map((d,i)=>{
          ...
          const val = isPacker && !expandPacker ? d.value.slice(0,40)+"…" : d.value;
          return (
            <div key={i} style={{
              display:"flex", gap:0,
              borderBottom:"1px solid #f1f5f9",
              padding:"12px 0",
              gridColumn: isPacker ? "1/-1" : undefined,
              animation:`fadeUp .35s ease ${i*.04}s both`,
            }}>
              <div style={{ width:180, flexShrink:0, fontSize:12, fontWeight:700, color:"#64748b", textTransform:"uppercase", letterSpacing:"0.5px", paddingRight:16 }}>{d.label}</div>
              <div style={{ fontSize:13, color:"#0f172a", lineHeight:1.6 }}>
                {val}
                {isPacker&&(
                  <button onClick={()=>setExpandPacker(p=>!p)} style={{ marginLeft:8, background:"none", border:"none", color:"#6366f1", cursor:"pointer", fontSize:12, fontWeight:600, padding:0 }}>
                    {expandPacker?"Show less":"Show more"}
                  </button>
                )}
              </div>
            </div>
